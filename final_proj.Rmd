...
stage_bar <- ggplot(cholangitis, aes(x=stage, color=factor(stage), fill=factor(stage))) +
  geom_bar() + 
  theme(legend.position="none") +
  labs(title = "Bar Plot of Disease Status")
...
ggplot(cholangitis, aes(x=status, y=n_days)) + 
  ...
  labs(title = "Box Plot of N_Days against disease status")
```
...
```
Based on this mosaic plot, the ratio of people who received the placebo that survived is higher than those who received the trial drug, but they are relatively close. This suggests that the drug did not have a great impact on the outcome of the patient across all dimensions. 

```{r medical indicators bar plot} 
ascites_bar <- ggplot(cholangitis, aes(x=ascites, color=factor(ascites), fill=factor(ascites))) +
  geom_bar() + 
  theme(legend.position="none") +
  labs(title = "Bar Plot of prescence of Ascites")

hepatomegaly_bar <- ggplot(cholangitis, aes(x=hepatomegaly, color=factor(hepatomegaly), fill=factor(hepatomegaly))) +
  geom_bar() + 
  theme(legend.position="none") +
  labs(title = "Bar Plot of prescence of Hepatomegaly")

spiders_bar <- ggplot(cholangitis, aes(x=spiders, color=factor(spiders), fill=factor(spiders))) +
  geom_bar() + 
  theme(legend.position="none") +
  labs(title = "Bar Plot of prescence of Spider angiomas")

edema_bar <- ggplot(cholangitis, aes(x=edema, color=factor(edema), fill=factor(edema))) +
  geom_bar() + 
  theme(legend.position="none") +
  labs(title = "Bar Plot of Prescence of Edema")
